...
describe 'Titanic' do
  it 'persists requests to disk and serves results asynchronously' do
    ...
    Async do |task|
      rep = OMQ::REP.bind(frontend_ep)
      rep.recv_timeout = 2
      push = OMQ::PUSH.bind(dispatch_ep)

      frontend_task = task.async do
        loop do
      ...
      pull = OMQ::PULL.connect(dispatch_ep)
      pull.recv_timeout = 2

      dispatcher_task = task.async do
        loop do
      ...
      client = OMQ::REQ.connect(frontend_ep)
